...
// ===== AUDIO HELPERS =====
async function getAudioUrl(base64Map, letter) {{
...
async function playLetterAudio(letter) {{
  const url = await getAudioUrl(LETTER_AUDIO, letter);
  await playAudio(url);
}}

async function playWordAudio(letter) {{
  const url = await getAudioUrl(WORD_AUDIO, letter);
  await playAudio(url);
}}

async function playSentAudio(letter) {{
  const url = await getAudioUrl(SENT_AUDIO, letter);
  await playAudio(url);
}}
